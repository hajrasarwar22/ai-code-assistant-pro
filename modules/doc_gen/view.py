    ...
    prompt = [
        {
            ...
            "content": (
                ...
                "Be thorough, accurate, and professional. Use Markdown formatting where appropriate."
            )
        },
        {
            "role": "user",
            "content": f"Generate documentation for:\n\n```{LANG_SLUG.get(lang, 'text')}\n{code}\n```"
        },
    ]
...
def doc_gen_view():
    ...
    with dc1:
        st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;color:#4B6280;
            ...
            ◈ &nbsp;Documentation Type</div>""", unsafe_allow_html=True)
        doc_type = st.selectbox("doctype", DOC_TYPES, label_visibility="hidden", key="doc_type")
    ...
    with dc2:
        ...
        lang = st.selectbox("lang", list(LANG_SLUG.keys()),
                            label_visibility="hidden", key="doc_lang")

    with st.form("doc_gen_form"):
        st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;color:#4B6280;
            letter-spacing:.16em;text-transform:uppercase;margin-bottom:.4rem;">
            ◈ &nbsp;Your Code</div>""", unsafe_allow_html=True)
    ...
    if submitted and code:
        ...
        t1, t2 = st.tabs(["📝 Documentation", "📄 Your Code"])

        with t1:
            st.markdown(f"""<div class="rh">
                <div class="rb rb-b"></div>
                <span class="rl">Generated Docs</span>
                <span class="badge bb" style="margin-left:.4rem;">{doc_type.split('(')[0].strip()}</span>
        ...
        st.markdown('<div class="ie">⚠ Please paste code to document.</div>',
                    unsafe_allow_html=True)
