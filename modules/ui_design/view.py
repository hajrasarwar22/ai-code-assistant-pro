...
def ui_design_view():
    ...
    <div class="hero hero-blue">
        ...
        <div class="hero-sub">Describe your UI → get responsive, styled code with design rationale</div>
    </div>
    ...
    st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;
        ...
        margin-bottom:.45rem;">◈ &nbsp;Quick Preset</div>""", unsafe_allow_html=True)

    preset = st.selectbox("preset", PRESETS, label_visibility="hidden", key="ui_preset")

    pc1, pc2, pc3 = st.columns(3)
    with pc1:
        st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;
            color:#4B6280;letter-spacing:.16em;text-transform:uppercase;
            margin-bottom:.4rem;">◈ &nbsp;Design Style</div>""", unsafe_allow_html=True)
        style_pref = st.selectbox("style", STYLES, label_visibility="hidden", key="ui_style")

    with pc2:
        st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;
            color:#4B6280;letter-spacing:.16em;text-transform:uppercase;
            margin-bottom:.4rem;">◈ &nbsp;Theme</div>""", unsafe_allow_html=True)
        theme_pref = st.selectbox("theme", THEMES, label_visibility="hidden", key="ui_theme")

    with pc3:
        st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;
            color:#4B6280;letter-spacing:.16em;text-transform:uppercase;
            margin-bottom:.4rem;">◈ &nbsp;Framework</div>""", unsafe_allow_html=True)
        fw_pref = st.selectbox("framework", FRAMEWORKS, label_visibility="hidden", key="ui_fw")

    with st.form("ui_design_form"):
        st.markdown("""<div style="font-family:'Fira Code',monospace;font-size:.63rem;
            color:#4B6280;letter-spacing:.16em;text-transform:uppercase;
            margin-bottom:.4rem;">◈ &nbsp;Describe Your UI</div>""", unsafe_allow_html=True)

        default_idea = "" if preset == "Custom..." else preset
        user_idea = st.text_area("idea", height=140,
                                 placeholder="e.g. A dark-themed admin dashboard with sidebar, KPI cards, charts...",
                                 value=default_idea, label_visibility="hidden")
    ...
    if submitted and user_idea:
        ...
        with t1:
            st.markdown("""<div class="rh"><div class="rb rb-b"></div>
                <span class="rl">Generated UI Code</span></div>""", unsafe_allow_html=True)
            st.code(code_part, language="html")

        with t2:
            st.markdown("""<div class="rh"><div class="rb rb-b"></div>
                <span class="rl">Design Notes</span></div>""", unsafe_allow_html=True)
            st.markdown(notes_part)

    elif submitted and not user_idea:
        st.markdown('<div class="ie">⚠ Please describe the UI you want to generate.</div>',
                    unsafe_allow_html=True)
